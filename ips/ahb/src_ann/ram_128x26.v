/*
    ...
    Configuration: -instname ram_128x26 -rows 32 -bits 26 -mux 4 
    Redundancy: Off
    Bit-Write: Off
*/
...
/*
  specify
    ...
    $period(posedge CLKA,1.722,CLKA_CYC_flag);
    $width(posedge CLKA,0.517,0,CLKA_H_flag);
    $width(negedge CLKA,0.517,0,CLKA_L_flag);
    
    $setuphold(posedge CLKB,posedge CENB,1.000,0.500,CENB_flag);
    $setuphold(posedge CLKB,negedge CENB,1.000,0.500,CENB_flag);
    $period(posedge CLKB,1.722,CLKB_CYC_flag);
    $width(posedge CLKB,0.517,0,CLKB_H_flag);
    $width(negedge CLKB,0.517,0,CLKB_L_flag);
